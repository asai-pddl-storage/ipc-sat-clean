...
(define (domain TIDYBOT)
  ...
  (:types robot cart thing xc yc xrel yrel)
  (:predicates
   ;; Constant preds
   (leftof                   ?x1 - xc ?x2 - xc)
   (above                    ?y1 - yc ?y2 - yc)
   ...
   ;; Objects
   (thing-pos   ?o - thing ?x - xc  ?y - yc)
   (thing-done  ?o - thing)
   (surface ?x - xc ?y - yc)

   ;; Gripper
   (holding       ?r - robot ?o - thing)
  ...
  (:action get-left
   ...
   :precondition (and (parked ?r)
                      ...
                      (gripper-empty ?r)
                      (leftof ?ox ?gxabs)
                      (not (thing-done ?o)) (thing-pos ?o ?ox ?gyabs))
   :effect       (and (not (thing-pos ?o ?ox ?gyabs))
  ...
  (:action get-up
   ...
   :precondition (and (parked ?r)
                      ...
                      (gripper-empty ?r)
                      (above ?oy ?gyabs)
                      (not (thing-done ?o)) (thing-pos ?o ?gxabs ?oy))
   :effect       (and (not (thing-pos ?o ?gxabs ?oy))
  ...
  (:action put-on-cart
   :parameters (?r - robot ?x - xc ?y - yc ?gxrel - xrel 
                ?gyrel - yrel ?o - thing ?c - cart ?cx - xc ?cy - yc)
  ...
  (:action finish-thing
   :parameters (?o - thing ?x - xc ?y - yc)
   :precondition (and (not (thing-done ?o)) (thing-pos ?o ?x ?y) (thing-goal ?o ?x ?y))
   :effect (and (thing-done ?o)))
)
